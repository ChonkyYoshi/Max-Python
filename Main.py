...
def Doc2PDF(PathInput, AccTC, RejTC, DelCom, Overwrite):
	...
	FullPath = fn.Doc2PDF(FullPath, PathOnly, FileOnly, AccTC, RejTC, DelCom, Overwrite)
...
def AcceptRevisions(PathInput, AccTC, RejTC, DelCom, Overwrite):
	...
	MainWindow['---PBar---'].update(index/len(PathList)*100)
	FullPath = fn.AcceptRevisions(FullPath, PathOnly, FileOnly, AccTC, RejTC, DelCom, Overwrite)

#GUI layout and creation
TopText = [
	[gui.Text(text=str(config['Descriptions']['TopText']),justification='center')],
...
Rightlayout = [
	[gui.InputText(default_text='',key='---PathInput---',),gui.FilesBrowse(target='---PathInput---')],
	[gui.Text(text='',key='---Description---')],
	[gui.Submit(button_text='Run',size=15,visible=False,key='---Run---')],
	[gui.Checkbox(text='Accept Revisions',auto_size_text=True,key='---AcceptTC---',visible=False), gui.Checkbox(text='Reject Revisions',auto_size_text=True,key='---RejectTC---',visible=False)],
	[gui.Checkbox(text='DeleteComments',auto_size_text=True,key='---DeleteCom---',visible=False)],
	[gui.Checkbox(text='Overwrite',auto_size_text=True,key='---Overwrite---',visible=False)],
	[gui.ProgressBar(max_value=100,orientation='horizontal',size=(50,20),bar_color=('green','white'),key='---PBar---',visible=False)],
	[gui.Text(text='',key='---PBarFile---')],
	[gui.Text(text='',key='---PBarFileStep---')]
]

layout = [[TopText, gui.Column(Sidebar, vertical_scroll_only=True, scrollable=True,expand_y=True), gui.VSeparator(), gui.Column(Rightlayout)]]
...
while True:
	...
	match event:
		...
		case gui.WIN_CLOSED:
			break
		case '---Run---':
			if MainWindow['---AcceptTC---'].get() == True and MainWindow['---RejectTC---'].get() == True:
				gui.popup_error('Both Accept and Reject TC are selected!\nPlease check only of the options and try again.',title='Both accept and reject selected')
				break
			...
			for index, PathInput in enumerate(PathList):
				...
				match FunctionName:
					...
					case 'Bilingual_Table':
						Bilingual(PathInput)
					case 'Doc2PDF':
						Doc2PDF(PathInput, MainWindow['---AcceptTC---'].get(), MainWindow['---RejectTC---'].get(), MainWindow['---DeleteCom---'].get(), MainWindow['---Overwrite---'].get())
					case 'Accept Revisions':
						AcceptRevisions(PathInput, MainWindow['---AcceptTC---'].get(), MainWindow['---RejectTC---'].get(), MainWindow['---DeleteCom---'].get(), MainWindow['---Overwrite---'].get())
		...
		case 'Accept Revisions':
			MainWindow['---Description---'].update(config['Descriptions']['Accept_Revisions'])
			MainWindow['---Run---'].update(visible=True)
			MainWindow['---AcceptTC---'].update(visible=True)
			MainWindow['---RejectTC---'].update(visible=True)
			MainWindow['---DeleteCom---'].update(visible=True)
